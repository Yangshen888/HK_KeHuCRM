...
using HaikanCRM.Api.Entities.User;
...
using Haikan3.Utils;
using UnifyService;
using static UnifyService.PassportServiceSoapClient;
    ...
    public class OauthController : ControllerBase
    {
        ...
        public IActionResult WXAuth2(UserData userdata)
        {
            ...
            using (_dbContext)
            {
                ...
                //if (user.PassWord != Haikan3.Utils.DesEncrypt.Encrypt(userdata.password.Trim(), MdDesEncrypt.SecretKey))
                if (user.PassWord != (userdata.password.Trim()))
                {
                    response.SetFailed("密码不正确");
                    return Ok(response);
                }
...
//                    var pagePermissions = permissions.GroupBy(x => x.MenuAlias).ToDictionary(g => g.Key, g => g.Select(x => x.PermissionActionCode).Distinct());
        ...
        /// 统一身份登录
        /// </summary>
        /// <param name="unifyToken"></param>
        /// <returns></returns>
        [Route("/api/v1/Oauth/UnifyAuth")]
        [HttpGet("unifyToken")]
