    ...
    <Card>
      ...
      >
        ...
        <Table
          ...
          @on-page-size-change="handlePageSizeChanged"
        >
          <!-- <template slot-scope="{row,index}" slot="isFixation">
            <span>{{renderStatus(row.isFixation)}}</span>
          </template>-->
          <!-- <template slot-scope="{row,index}" slot="isFixation">
            <Tag :color="renderStatus(row.isFixation).color">{{renderStatus(row.isFixation).text}}</Tag>
          </template> -->
...
export default {
      ...
      stores: {
        role: {
          ...
          columns: [
            ...
            { title: "角色名称", key: "roleName",minWidth:100, sortable: true },
            ...
            { title: "创建者", key: "addPeople",minWidth:100,maxWidth:130, },
            {
              title: "操作",
              align: "center",
              width: 130,
  ...
  methods: {
    ...
    handleEdit(row) {
      ...
      this.doLoadRole(row.systemRoleUuid);
    },
    ...
    handleDelete(row) {
      this.doDelete(row.systemRoleUuid);
    },
    doDelete(ids) {
      if (!ids) {
