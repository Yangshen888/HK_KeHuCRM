          ...
          <template slot-scope="{ row, index }" slot="action">
            <Poptip confirm :transfer="true" title="确定要删除吗?" @on-ok="handleDelete(row)">
              <Tooltip placement="top" content="删除" :delay="1000" :transfer="true">
                <Button type="error" size="small" shape="circle" icon="md-trash" v-can="'delete1'"></Button>
              </Tooltip>
            </Poptip>
    ...
    >
      <Form :model="formModel.fields" ref="formUser" :rules="formModel.rules" label-position="top">
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="登录名" prop="loginName">
              <Input v-model="formModel.fields.loginName" :readonly="formModel.mode == 'edit'" placeholder="请输入登录名" />
            </FormItem>
          </Col>
          <Col span="12">
        ...
        <Row :gutter="16">
          ...
          <Col span="12">
            <FormItem label="电子邮箱" prop="oldCard">
              <Input v-model="formModel.fields.oldCard" placeholder="请输入电子邮箱" />
            </FormItem>
          </Col>
          <!-- <Col span="12">
...
import {globalvalidateEmail} from "../../global/validate.js"
...
export default {
  ...
  data() {
    let globalvalidatePhone = (rule, value, callback) => {
      if (value !== "" && value !== null) {
        let reg = /(^bai\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
        if (!reg.test(value)) {
    ...
    return {
      ...
      formModel: {
        ...
        rules: {
          ...
          //password: [],
          oldCard: [
            { type: "string", required: true, validator: globalvalidateEmail }
          ],
        }
      },
      formAssignRole: {
      ...
      stores: {
        user: {
          ...
          columns: [
            ...
            { title: "登录名", key: "loginName",minWidth:150,maxWidth:200, sortable: true },
            { title: "真实名", key: "realName",minWidth:80,maxWidth:100, },
            { title: "身份证号", key: "userIdCard",minWidth:140, },
  ...
  methods: {
    ...
    doCreateUser() {
      ...
      // }
      let regn = /^[^\s]+$/;
      if (!regn.test(this.formModel.fields.loginName)) {
        this.$Message.warning("请输入有效的名称");
        return;
      }
      let regnn = /^[^\s]+$/;
      if (!regnn.test(this.formModel.fields.passWord)) {
        this.$Message.warning("请输入有效的密码");
        return;
      }
      console.log(this.formModel.fields);
    ...
    tongbu() {
      this.$Modal.confirm({
        title: "操作提示",
        content: "<p>确定要同步钉钉的人员信息和统一身份信息吗?</p>",
        loading: true,
        onOk: () => {
