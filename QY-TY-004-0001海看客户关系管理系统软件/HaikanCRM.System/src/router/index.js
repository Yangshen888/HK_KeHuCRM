...
router.beforeEach((to, from, next) => {
  ...
  let isUnifyAuth=config.haikanPassport_IfUse;
  console.log(isUnifyAuth);
  if (!token && to.name !== LOGIN_PAGE_NAME) {

    

    console.log(localStorage.getItem('xxxx'));
    if(localStorage.getItem('xxxx')==null){
    ...
    if (isUnifyAuth&&Object.keys(to.query).length <= 0&&localStorage.getItem('xxxx')=="0") {
      localStorage.setItem('xxxx',"1")
      window.location.href = 'https://sso.haikan.com.cn/userlogin.aspx?backurl=' + encodeURIComponent(config.webUrl+`/#/login/`).toLowerCase();

    }
    else if(isUnifyAuth&&Object.keys(to.query).length<=0&&localStorage.getItem('xxxx')=="1"){
      localStorage.setItem('xxxx',"2");
      window.location.href='https://sso.haikan.com.cn/gettoken.aspx?backurl='+ encodeURIComponent(config.webUrl+`/#/login/?token=$token$`).toLowerCase();
    } 
    else {
      next({
