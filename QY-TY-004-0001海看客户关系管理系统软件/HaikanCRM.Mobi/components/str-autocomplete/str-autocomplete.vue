<template>
    <view>
        <input class="uni-input" placeholder="请输入内容"  style="font-size: 15px;text-align:right ;" :value="value" @input="onInput" @blur="hideList"/>
        <view  v-if="isShow">
            <view v-for="(item, index) in showList" :key="index" class="str-auto-complete-item" @tap="selectThisItem(item)">{{item.showString}}</view>
        </view>
    </view>
</template>
