<template>
	<view>
		<form>
			<view class="cu-card article no-card" style="height: 250px;">
				<view class="cu-item shadow-warp bg" style="height: 100%;">
					<view class="title">
						...
						<view class="radius" style="margin-top: 6px;margin-left: 35%;" @click="addmation">
			...
			<view class="cu-list ">
				<view class="cu-form-group inputlist">
					...
					<input name="input" style="text-align:right" disabled="disabled" v-model="list.bumenName==null?'暂未加入部门':list.bumenName"/>
				</view>
				<view class="cu-form-group inputlist">
			...
			<view class="cu-list " style="height: 170px;">
				<view class="cu-avatar round lg margin-xs bg-olive"  style="margin: 10px;margin-left: 20px;" @click="person">
					<text class="avatar-text">{{clientnum}}</text>
				</view>
				<view class="cu-avatar round lg margin-xs bg-blue"  style="margin: 10px;background-image: none;background-color: rgb(0,129,255);" @click="business">
...
import { PersonalList } from '@/api/information/information.js';
	export default {
		...
		methods:{
			...
			business(){
				uni.navigateTo({
					url: '/pages/duoduoCRM/business/business'
				});
			},
			addmation(){
				let data=getApp().globalData.SystemUserUuid;
				uni.navigateTo({
					url: '/pages/duoduoCRM/information/addmation?SystemUserUuid='+data
				});
			}
		},
		mounted(opt){
